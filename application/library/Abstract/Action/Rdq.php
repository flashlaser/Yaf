...
class Abstract_Action_Rdq extends Yaf_Action_Abstract{
    ...
    final public function execute() {
        ...
        // 参数
        $controller_name = $this->getRequest ()->getControllerName (); // 控制器名称
        $action_name = $this->getRequest ()->getActionName (); // 控制器名称
        $idx = $this->getRequest ()->getParam ( 'proc_num' ); // 进程编号
        if ($idx < 1) {
            throw new Exception_System ( 200301, "进程编号小于1", array (
        ...
        // 检查uri是否合法
        if ("Cli/{$controller_name}/{$action_name}/proc_num/{$idx}" != ltrim ( $controller->getRequest ()->getRequestUri (), '/' )) {
            throw new Exception_System ( 200305, null, array (
                    'right' => "Cli/{$controller_name}/{$action_name}/proc_num/{$idx}",
        ...
        // 心跳
        $last_beat_time = 0;
        $beat_file = $file = sprintf ( Comm_Config::get ( 'rdq.pub_conf.beat_file_path' ), $_SERVER['SRV_PRIVDATA_DIR'], $action_name, $idx );
        ...
        while ( true ) {
            // 检查是否有停止命令
            if ($this->getController ()->checkStop ( 'rdq', $action_name, $idx ) === 'stop') {
                break;
            }
            ...
            $data = Comm_Rdq::read ( $mcq_name, true, Helper_Debug::isDebug () ? 1 : $this->max_num );
            if (! $data) { // 一条数据也没有获取到
                ...
                $this->show_deal_time && $this->printf ( 'Start at:%s', date ( 'Y-m-d H:i:s' ) );
                ...
                $this->show_deal_time && $this->printf ( 'Finished at:%s', date ( 'Y-m-d H:i:s' ) );
                $this->show_deal_time && $this->printf ( 'Mq name:%s, Deal num:%s, Use time:%s', $mcq_name, $deal_num, $microtime );
            }
        }
    ...
    final protected function getMcqName($action_name, $idx) {
        $arr = Comm_Config::get ( 'rdq.rq_list' );
        ...
        if (is_null ( $proc_total )) {
            throw new Exception_System ( 200301, "mcq($action_name,$idx)未配置，禁止运行", array (
                    'file' => __FILE__,
                    'line' => __LINE__ 
            ) );
        }
        ...
        if ($idx < 1 || $idx > $proc_total) {
            throw new Exception_Msg( 200301, "mcq($action_name,$idx)在配置范围[1,{$arr[$action_name]['proc_total']}]外，禁止运行", array (
                    'file' => __FILE__,
                    'line' => __LINE__ 
            ) );
        }
     ...
     */
    protected function process(array $datas) {
    }
    ...
    public function output($text) {
        ...
        $this->checkLogMaxLine ( $filename, $this->max_line );
