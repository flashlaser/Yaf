 ...
 */
class RdqAction extends Yaf_Action_Abstract {
     ...
     * @var integer
     */
    protected $_die_time = 600;//10分钟，死亡时间，单位：秒。进程无心跳超过此时间则判断为死亡，若还有进程强制杀死
    ...
    protected function config() {
        ...
        $tmp = Comm_Config::get('rdq.rq_list');
        foreach ($tmp as $mcq_name => $mcq_info) {
            if ($mcq_info['action_name']) {
                $this->_config_data[$mcq_info['action_name']] = array(
                    'mcq_name'   => $mcq_name,
    ...
    protected function checkRuning() {
        ...
        echo "\033[35mCheck RDQ runing:\r\n\033[0m";
        foreach ($this->_config_data as $action_name => $arr) {

            for ($idx = 1; $idx <= $arr['proc_total']; $idx++) {
                $request_uri = "/Cli/Rdq/{$action_name}/proc_num/{$idx}";
                echo "\n{$request_uri}\n";
                //检查进程是否存在
    ...
    protected function checkModify() {
        $code_dir = APP_PATH . '/application/modules/Cli/actions/Rdq/';//代码目录
        $bak_dir  = $_SERVER['SRV_PRIVDATA_DIR'] . '/rdq_check_desc/';//文件备份目录。备份最后的代码文件，用于比对是否修改过
        if (!is_dir($bak_dir)) {
            mkdir($bak_dir, 0775, true);
        }

        echo "\033[35mCheck RDQ modify:\r\n\033[0m";
        ...
        foreach ($this->_config_data as $action_name => $arr) {
            ...
            if ($str_code != $str_bak) { //文件有改动，发送停止消息
                for ($idx = 1; $idx <= $arr['proc_total']; $idx++) {
                    $this->getController()->sendStop(self::TYPE, $action_name, $idx);
                }
    ...
    protected function checkBeat($action_name, $idx, $shell) {
        $file = sprintf(Comm_Config::get('rdq.pub_conf.beat_file_path')
        ...
        if (NOW - $last_beat_time > $this->_die_time) {
    ...
    protected function checkRsyncStop($uri, $action_name, $idx) {
        ...
        return $this->getController()->setStop(self::TYPE, $action_name, $idx);
    }

    /**
     * 获取监控后台推送的，需要停止的mcq进程的uri列表
     * 文件内容列样例：/Cli/Rdq/Channelsync/proc_num/1
     * 
     * @return array
     */
    protected function getStopUri() {
        ...
        $file = $_SERVER['SRV_PRIVDATA_DIR'] . '/monitor/reboot_rdq_daemon.txt';
